...
import sys
try:
    from urlparse import urlsplit, parse_qs, unquote_plus, quote_plus
except ImportError:
    from urllib.parse import urlsplit, parse_qs, unquote_plus, quote_plus
try:
...
class Plugin:
    ...
    def url_for_path(self, path):
        ...
        pathsps = path.split('/')
        path=''
        for pathsp in pathsps:
            path+='/'+(pathsp).replace('&', '%26').replace(',', '%2C')
        #path = path if path.startswith('/') else '/' + path
        #xbmc.log('zzzzzz:'+path, level=xbmc.LOGDEBUG)
        ...
        for view_func, rules in iter(list(self._rules.items())):
            for rule in rules:
                ...
                if kwargs is not None:
                    log("path:%s Dispatching to '%s', args: %s" % (path,view_func.__name__, kwargs))
                    view_func(**kwargs)
                    return
...
class UrlRule:
    ...
    def match(self, path):
        ...
        if match:
            items = dict((key, val.replace('%26', '&').replace('%2C', ','))
    ...
    def make_path(self, *args, **kwargs):
        ...
        url_kwargs = dict(((k, str(v).replace('&', '%26').replace(',', '%2C')) for k, v in list(kwargs.items()) if k in self._keywords))
        qs_kwargs = dict(((k, str(v).replace('&', '%26').replace(',', '%2C')) for k, v in list(kwargs.items()) if k not in self._keywords))

        query = '?' + urlencode(qs_kwargs) if qs_kwargs else ''
        log('zzzzz：'+self._pattern.format(**url_kwargs) + query)
        try:
