...
plugin=Plugin()
class ciliwiki(object):
	url = 'https://www.ciliwiki.com'
	name = 'ciliwiki'
	...
	def search(self, what, cat='all',sorttype='relevance',page='1'): 	
		...
		try:
			...
			rmain=r'\x3Ch3\x3E.*?hash\x2F(?P<magnet>.*?)\x2Ehtm.*?\x3E(?P<title>.*?)\x3C\x2Fa\x3E.*?创建时间：\x3Cb\x3E(?P<createtime>.*?)\x3C\x2Fb.*?文件大小：\x3Cb.*?\x3E(?P<filesize>.*?)\x3C\x2Fb.*?文件数量：\x3Cb.*?\x3E(?P<filecount>.*?)\x3C\x2Fb.*?下载热度：\x3Cb\x3E(?P<pop>.*?)\x3C\x2Fb'
