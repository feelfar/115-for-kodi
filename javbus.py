...
from commfunc import keyboard,_http,url_is_alive,encode_obj,notify,get_setting,get_storage,ListItem,add_items,set_resolved_url
colorize_label=comm.colorize_label

javbusurl = get_storage('javbusurl')
'''
javbusurl['base']=get_setting('javbusqb').lower()		
javbusurl['qb']=get_setting('javbusqb').lower()
javbusurl['bb']=get_setting('javbusqb').lower()+'/uncensored'
'''
javbusurl['om']=get_setting('javbusom').lower()
...
def javbus():
    ...
    menus = [
        ...
        {'label': '片片找找', 'path':  plugin.url_for_path('searchinit/%s/%s/%s/%s'%('jav','0','0','{}'))},
...
def getjavbusurl():
    ...
    for url in get_setting('javbusfb').lower().split(';'):
    ...
    urls=[]
    try:
        javbusfb=get_setting('javbusfb').lower().split(';')
        rsp = _http(get_setting('javbusfb').lower())
...
def javmagnet(qbbb='qb',gid='0',uc='0'):
    ...
    for match in leechmagnet.finditer(rspmagnet):
        ...
        listitem=ListItem(label=comm.colorize_label(title, 'bt'), 
            ...
            path=plugin.url_for_path('execmagnet/%s/%s/%s'%(magnet,title,filemsg)))
...
def javdetail(qbbb='qb',movieno='0',id='0',title='0'):
    ...
    context_menu_items.append(('搜索'+colorize_label(id, color='00FF00'), 
        'RunPlugin('+plugin.url_for(searchinit,stypes='pan,bt',sstr=id,modify='1',otherargs='{}')+')',))
    ...
    context_menu_items=[]
    context_menu_items.append(('搜索'+colorize_label(title, color='00FF00'), 
        'RunPlugin('+plugin.url_for(searchinit,stypes='pan,bt',sstr=title,modify='1',otherargs='{}')+')',))
        
    listitem=ListItem(label='BT:[COLOR FF00FFFF]%s[/COLOR]' % (title),
            thumbnail=xbmc.translatePath( os.path.join( __cwd__, 'magnet.jpg') ), 
            path=plugin.url_for(btsearchinit, sstr=title, modify='0'),)
            
    if len(context_menu_items)>0 and listitem!=None:
        listitem.add_context_menu_items(context_menu_items)
        menus.append(listitem)
    '''
    searchiniturl=plugin.url_for_path('searchinit/%s/%s/%s/%s'%('pan,bt',id,'1','{}'))
    menus.append(ListItem(label='搜索:[COLOR FF00FFFF]%s[/COLOR]' % (id),
            ...
            path=searchiniturl,))
    searchiniturl=plugin.url_for_path('searchinit/%s/%s/%s/%s'%('pan,bt',title,'1','{}'))
    ...
    for match in leech.finditer(rsp):
        ...
        menus.append({'label':'封面图',
              'path': plugin.url_for_path('showpic/%s'%(match.group('mainimg'))),
    ...
    for match in leech.finditer(rsp):
        ...
        if filterkey and filterkey!='0':
            ...
            if match.group('filter_type')=='series':
                ...
                filtertypename='系列'
            if filtertype:
                searchiniturl=plugin.url_for_path('searchinit/%s/%s/%s/%s'%(('pan,bt,jav'),(filtername),'1','{}'))
    ...
    for match in leech.finditer(rsp):
        context_menu_items=[]
        searchiniturl=plugin.url_for_path('searchinit/%s/%s/%s/%s'%(('pan,bt,jav'),(match.group('starname')),'1','{}'))
    ...
    for match in leech.finditer(rsp):
        ...
        menus.append({'label':'样品图',
              'path': plugin.url_for_path('showpic/%s'%(match.group('sampleimg'))),
...
def javlist(qbbb='qb',filtertype='0',filterkey='0',page=1):
    ...
    try:
        ...
        for match in leech.finditer(rsp):
            ...
            context_menu_items=[]
            searchiniturl=plugin.url_for_path('searchinit/%s/%s/%s/%s'%(('pan,bt'),(match.group('id')),'1','{}'))
...
def javstarlist(qbbb='qb',page=1):
    ...
    try:
        ...
        for match in leech.finditer(rsp):
            context_menu_items=[]
            searchiniturl=plugin.url_for_path('searchinit/%s/%s/%s/%s'%(('pan,bt'),match.group('starname'),'1','{}'))
            context_menu_items.append(('搜索'+colorize_label(match.group('starname'), color='00FF00'), 
                'RunPlugin('+searchiniturl+')',))
