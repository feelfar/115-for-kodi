...
@plugin.route('/javbus')
def javbus():
	if not javbusurl.raw_dict().has_key('base'):
		getjavbusurl()
...
def javdetail(qbbb='qb',movieno='0',id='0',title='0'):
	...
	context_menu_items=[]
	context_menu_items.append(('搜索'+colorize_label(title.encode('UTF-8'), color='00FF00'), 
		'RunPlugin('+plugin.url_for('searchinit',stypes='pan,bt',sstr=title.encode('UTF-8'),modify='1',otherargs='{}')+')',))
		
	listitem=ListItem(label='BT:[COLOR FF00FFFF]%s[/COLOR]' % (title.encode('utf-8')),
			thumbnail=xbmc.translatePath( os.path.join( __cwd__, 'magnet.jpg') ).decode('utf-8'), 
		...
		if filterkey and filterkey!='0':
			...
			if filtertype:
				menus.append({'label':'%s:%s'%(filtertypename,filtername),
					  ...
					  'context_menu':[('搜索'+colorize_label(filtername.encode('UTF-8'), color='00FF00'), 
						'RunPlugin('+plugin.url_for('searchinit',stypes='pan,bt,jav',sstr=filtername.encode('UTF-8'),modify='1',otherargs='{}')+')',)]
					  })
	...
	for match in leech.finditer(rsp):
		...
		context_menu_items.append(('搜索'+colorize_label(match.group('starname'), color='00FF00'), 
			'RunPlugin('+plugin.url_for('searchinit',stypes='pan,bt,jav',sstr=match.group('starname'),modify='1',otherargs='{}')+')',))
...
def freepv(movieid=''):
	videourl=''
	subpath=xbmc.translatePath( os.path.join( __cwd__, 'sample.m3u8') ).decode('utf-8')
	...
	if movieid[0:3]=='bb,':
		...
		for mid in [movieid,movieid[0:-5]+movieid[-3:]]:
			...
			for stm in ['_dmb_w','_dm_w','_sm_w','_dmb_s','_dm_s','_sm_s']:
				...
				if comm.url_is_alive(videourltemp):
...
def javlist(qbbb='qb',filtertype='0',filterkey='0',page=1):
	if not javbusurl.raw_dict().has_key('base'):
		getjavbusurl()
	if not javbusurl.raw_dict().has_key('existmag'):
		javbusurl['existmag']='all'
	filterkey=filterkey.replace(' ','')
	filterkey=urllib.quote(filterkey)
	filter=''
	if filtertype!='0':
		if filterkey!='0':
	...
	try:
		...
		for match in leech.finditer(rsp):
			...
			context_menu_items=[]
			context_menu_items.append(('搜索'+colorize_label(match.group('id').encode('UTF-8'), color='00FF00'), 
				'RunPlugin('+plugin.url_for('searchinit',stypes='pan,bt',sstr=match.group('id').encode('UTF-8'),modify='1',otherargs='{}')+')',))
...
def javstarlist(qbbb='qb',page=1):
	...
	try:
		...
		for match in leech.finditer(rsp):
			context_menu_items=[]
			context_menu_items.append(('搜索'+colorize_label(match.group('starname').encode('UTF-8'), color='00FF00'), 
				'RunPlugin('+plugin.url_for('searchinit',stypes='pan,bt',sstr=match.group('starname').encode('UTF-8'),modify='1',otherargs='{}')+')',))
